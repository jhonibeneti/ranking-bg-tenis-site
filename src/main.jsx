...
function buildRanking(groups, maxRound = Infinity) {
  const rows = [];
  groups.forEach((group) => {
    ...
    Object.entries(group.matches || {}).filter(([roundId]) => Number(roundId) <= maxRound).forEach(([, matches]) => matches.forEach((rawMatch) => {
...
function App() {
  ...
  const ranking = useMemo(() => data ? buildRanking(data.groups, rankingRound) : [], [data, rankingRound]);
    ...
    <div className="app-shell">
      ...
      <main className="content">
        <section className="section-block ranking-section" id="ranking">
          <div className="section-heading ranking-heading"><div><div className="eyebrow dark"><span className="eyebrow-line" />RANKING GERAL</div><h2>Os melhores jogadores</h2></div><label className="ranking-game-filter"><span>Ranqueando até</span><select aria-label="Filtrar ranking por rodada" value={String(rankingRound)} onChange={(event) => setRankingRound(Number(event.target.value))}>{rounds.map((round) => <option key={round.id} value={round.id}>{round.label}</option>)}</select></label></div>
        ...
        <section className="section-block method-section">
          ...
          <div className="method-note"><strong>Vitória = 3 pontos</strong><span>1. Saldo de vitórias</span><span>2. Confronto direto</span><span>3. Saldo de sets</span><span>4. Saldo de games</span><span>Match tiebreak = 1 game</span><span>O ranking parcial soma os resultados até a rodada selecionada e mantém todos os jogadores.</span></div>
        </section>
      </main>
